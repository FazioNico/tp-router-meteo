...
export const fetchMeteoData = async (city) => {
  ...
  const geocodingResponse = await fetch(`${import.meta.env.VITE_API_URL}/v1/search?name=${city}`);
  ...
  const meteoResponse = await fetch(`${import.meta.env.VITE_API_URL}/v1/forecast?latitude=${latitude}&longitude=${longitude}&current_weather=true`);
